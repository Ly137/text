...
<head>
    ...
    <title>基于GitHub的教学管理系统——实验记录</title>
...
<div class="page">
                    ...
                    <table class="table card-table table-vcenter text-nowrap">
                      ...
                      <c:forEach var="a" items="${alist }">
                        <tr>                                        
						  ...
						  <td class="text-right">
                            <a href="<%=basePath %>GetDataSvlt?id=${a.id }&tbname=stuTask"   class="btn btn-secondary btn-sm">详情</a>                            
                          </td>                                                                   
                        </tr>
                        </c:forEach>
                      </tbody>
            		...
            		<div class="col-auto ml-lg-auto">
		              <div class="row align-items-center">		                
		                <div class="col-auto">
					      ...
					      <div>
					      <a class=" btn btn-sm btn-outline-primary" href="javascript:void(0);" onclick="sybtdown('PagingSvlt?flag=1','mstutask');return false;"><i class="fe fe-chevrons-left"></i></a>
		                  <a class=" btn btn-sm btn-outline-primary" href="javascript:void(0);" onclick="syybtdown('PagingSvlt?flag=2','mstutask');return false;"><i class="fe fe-chevron-left"></i></a>
		                  <a class=" btn btn-sm btn-outline-primary" href="javascript:void(0);" onclick="xyybtdown('PagingSvlt?flag=3','mstutask');return false;"><i class="fe fe-chevron-right"></i></a>                
		                  <a class=" btn btn-sm btn-outline-primary" href="javascript:void(0);" onclick="wybtdown('PagingSvlt?flag=4','mstutask');return false;"><i class="fe fe-chevrons-right"></i></a>		              			               		             
		                 </div>
	...
	//查询
	function cktj(){
		var sqls="select teatask.title,teatask.remark,teatask.time,teatask.deadline,stutask.*,termname,classname,cname,tName from stutask,courseplan,teatask,classinfo,course,teacher,terms where classinfo.id=teatask.classinfo_id and teatask.terms_id=terms.id and teatask.teacher_id=teacher.id and  course.id=teatask.course_id and teatask.id=teatask_id";
		...
		$.ajax({
			...
			success:function(data){
				if(data.msg==1){
					window.location.href="<%=basePath%>InitSvlt?tbname=mstutask";
				}else{
					alert(data.msg);
				}
			}
