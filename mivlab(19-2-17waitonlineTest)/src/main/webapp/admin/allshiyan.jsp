...
<head>
    ...
    <title>基于GitHub的教学管理系统——教师实验记录</title>
...
<div class="page">
                    ...
                    <table class="table card-table table-vcenter text-nowrap">
                      ...
                      <c:forEach var="a" items="${alist }">
                        <tr>                         
						  ...
						  <td>${a.title }</td>
                          <td class="text-right">
                            <a href="<%=basePath %>GetDataSvlt?id=${a.id }&tbname=AteaTask" class="btn btn-secondary btn-sm">详情</a>                            
                          </td>                         
                        </tr>
                        </c:forEach>
                      </tbody>
            		...
            		<div class="col-auto ml-lg-auto">
		              <div class="row align-items-center">		                
		                <div class="col-auto">
					      ...
					      <div>
					      <a class=" btn btn-sm btn-outline-primary" href="javascript:void(0);" onclick="sybtdown('PagingSvlt?flag=1','allshiyan');return false;"><i class="fe fe-chevrons-left"></i></a>
		                  <a class=" btn btn-sm btn-outline-primary" href="javascript:void(0);" onclick="syybtdown('PagingSvlt?flag=2','allshiyan');return false;"><i class="fe fe-chevron-left"></i></a>
		                  <a class=" btn btn-sm btn-outline-primary" href="javascript:void(0);" onclick="xyybtdown('PagingSvlt?flag=3','allshiyan');return false;"><i class="fe fe-chevron-right"></i></a>                
		                  <a class=" btn btn-sm btn-outline-primary" href="javascript:void(0);" onclick="wybtdown('PagingSvlt?flag=4','allshiyan');return false;"><i class="fe fe-chevrons-right"></i></a>		              			               		             
		                 </div>
    ...
    <script type="text/javascript">
		...
		function cktj(){
			...
			if(teaid!="0"){
				sqls+="  and teacher.id="+teaid;
			}
			sqls+=" group by id order by id desc ";
			...
			$.ajax({
				...
				success:function(data){
					if(data.msg==1){
						window.location.href="InitSvlt?tbname=allshiyan";
					}else{
						alert(data.msg);
					}
				}
