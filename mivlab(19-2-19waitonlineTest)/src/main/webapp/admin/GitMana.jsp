...
<div class="page">
            ...
            <div class="row row-cards">
              <div class="col-12 card">              
		          ...
		          <div class="alert alert-icon " role="alert">
					  <i class="fe fe-bell mr-2" aria-hidden="true"></i> 统计前请先在“<a href="<%=basePath %>InitSvlt?flgs=1&tbname=adminInfo">设置</a>”中完善GitHub用户信息 
				  </div>
                  <div class="card-body">
                    <div class="row ">
    ...
    <script>
		...
		function submitGitdate(){
			var gitUsername='${userinfo.gitUsername}';
			var CLIENT_ID='${userinfo.CLIENT_ID}';
			var CLIENT_SECRET='${userinfo.CLIENT_SECRET}';
			var TOKEN='${userinfo.TOKEN}';
			if(gitUsername == "" || CLIENT_ID==""|| CLIENT_SECRET=="" || TOKEN==""){
				alert("GitHub用户信息不完整，无法统计,请在“设置”中完善信息");
				return ;
			}else{
				...
				if(org =="" || proj=="" || cktermsid=="0" || ccoursesid=="0" || pulls=="" || pullsFull=="" || commits=="" || commitsFull=="" || files=="" || filesFull=="" || comments=="" || commentsFull=="" || pullDone=="" 	|| pullGood==""  || pullLate=="" || pullCopy==""  || pullBad==""  || pullBug=="" ||  pullDemo==""  || pullInterest=="" 	){
					alert("请将信息填写完整");
					return ;
				}
				...
				$.ajax({
					...
					type:'post',
					data:{"org":org,"proj":proj,"uType":"admin","cktermsid":cktermsid,"ccoursesid":ccoursesid,"pulls":pulls,"pullsFull":pullsFull,"commits":commits,"commitsFull":commitsFull,"files":files,"filesFull":filesFull,"comments":comments,"commentsFull":commentsFull,"pullDone":pullDone,"pullGood":pullGood,"pullLate":pullLate,"pullCopy":pullCopy,"pullBad":pullBad,"pullBug":pullBug,"pullDemo":pullDemo,"pullInteresting":pullInterest},
					dataType:'json',
					success:function(data){			
							alert(data.msg);
							if(data.msg=="所有数据更新完毕"){
								window.location.href="<%=basePath %>GetDataSvlt?tbname=allGitDate&org="+org+"&proj="+proj;
							}			
					}
				});
			}
