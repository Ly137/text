                     ...
                     <div class="col-md-12 col-lg-12 ">                      	
                       <div class="card-body">	                    
                        <div class="row gutters-xs">
                          ...
                          <div class="col-4 col-lg-2 col-md-6">
                            <select id="ckcoursesid" class="form-control custom-select">
	   ...
	   function changeScore(){
		   ...
		   $.ajax({
				...
				success:function(data){	
					alert(data.msg);
					window.location.href="InitSvlt?tbname=allGitDate";
				}
			});
	   }
		...
		function gltj(){	//点击添加按钮，手动添加实验成绩信息
			...
			if(terms_id=="0"||course_id=="0"||org=="" ||proj=="" ||sno =="" || pulls==""||pullDone==""||pullGood==""||pullLate =="" ||pullCopy==""||pullBad==""||pullBug==""||pullDemo ==""|| pullInteresting=="" ||Commits==""|| ChangedFiles==""||  Comments=="" || score==""){
				...
				$.ajax({
					...
					success:function(data){
							alert(data.msg);	
							window.location.href="<%=basePath %>InitSvlt?tbname=allGitDate";
					}
				});
		...
		function cktj(){
			...
			var sqls="select students.sname, terms.termname,course.cname,classinfo.classname,"+tbname+".* from "+tbname+",students,course,terms,classinfo  where classinfo.id=students.classinfo_id and students.sno=Num and course_id=course.id and "+tbname+".terms_id=terms.id  ";
		...
		function delsbt(){
			...
			$.ajax({
				...
				success:function(data){
					window.location.href="<%=basePath %>InitSvlt?flgs=1&tbname=allGitDate";
					alert(data.msg);
				}
			});
			}
		...
		$(document).ready(function(){
				  ...
				  $(".alertTest").click(function(){
					  var jsonData = ${alistJson };
					    ...
					    let str = `班级,课程,姓名,学号,Pulls,Done,Good,Late,Copy,Bad,Bug,Demo,Interesting,Commits,Additions,Deletions,ChangedFiles,Comments,成绩\n`;
