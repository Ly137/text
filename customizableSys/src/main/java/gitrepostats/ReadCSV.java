...
public class ReadCSV {
	 public static void main(int tid,String org,String proj,int course_id,int terms_id){
	//未封装的CSV存入数据库函数
			//创建数据库操作对象
			...
			while (in.hasNext())
			{
				...
				if (temp.length == 14)
				{	
					boolean bl=db.checkTrue("select id from "+tablename+" where Num='"+temp[1]+"' and org='" +org+"' and proj='"+proj+"'");
					if(bl){
