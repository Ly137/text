...
<head>
    ...
    <title>基于GitHub的教学管理系统——原始实验数据</title>
...
<div class="page">
                       ...
                       <div class="card-body">	                    
                        <div class="row gutters-xs">
                          ...
                          <div class="col-4 col-lg-2 col-md-6">	
                          	<input type="search" class="form-control header-search"  id="ckuname" placeholder="请输入文件名&hellip;" tabindex="1">			                                                                                      	                             
                ...
                <div class="card">                 
                  <div class="table-responsive">
                    <table class="table card-table table-vcenter text-nowrap">
                      ...
                      <tbody>
                      <c:forEach var="a" items="${alist }" varStatus="index">
                        <tr>              
                          <td><input name="delid" type="checkbox" value="${a.id }"/></td>                                                  
		                  <td>${index.count }</td>
               			  <td><a >${a.filepath }</a></td>	                  
                          <td class="text-right">
                            <a href="${a.filepath }"  class="btn btn-secondary btn-sm">下载</a>                            
                          </td>                         
                        </tr>
                        </c:forEach>
                      </tbody>
            		...
            		<div class="col-auto ml-lg-auto">
		              <div class="row align-items-center">		                
		                <div class="col-auto">
					      ...
					      <div>
					      <a class=" btn btn-sm btn-outline-primary" href="javascript:void(0);" onclick="sybtdown('PagingSvlt?flag=1','TeagitRawdate');return false;"><i class="fe fe-chevrons-left"></i></a>
		                  <a class=" btn btn-sm btn-outline-primary" href="javascript:void(0);" onclick="syybtdown('PagingSvlt?flag=2','TeagitRawdate');return false;"><i class="fe fe-chevron-left"></i></a>
		                  <a class=" btn btn-sm btn-outline-primary" href="javascript:void(0);" onclick="xyybtdown('PagingSvlt?flag=3','TeagitRawdate');return false;"><i class="fe fe-chevron-right"></i></a>                
		                  <a class=" btn btn-sm btn-outline-primary" href="javascript:void(0);" onclick="wybtdown('PagingSvlt?flag=4','TeagitRawdate');return false;"><i class="fe fe-chevrons-right"></i></a>		              			               		             
		                 </div>
    ...
    <script type="text/javascript">   		   
		...
		//查询
		function cktj(){
		var sqls="select  * from  gitRawData  ";
		var un=$("#ckuname").val();
		if(un!=""){
			sqls+="where  filepath like '%"+un+"%' and tid="+${userid};
		}else{
			sqls+="where   tid="+${userid};
		}
		$.ajax({
			url:'CkSvlt',
			type:'post',
			...
			success:function(data){
				if(data.msg==1){
					window.location.href="InitSvlt?tbname=TeagitRawdate";
				}else{
					alert(data.msg);
				}
			}
		...
		$.ajax({
			...
			type:'post',
			dataType:'json',
			data:{"ids":ids,"tbname":"gitRawdate"},
			success:function(data){
				window.location.href="InitSvlt?tbname=TeagitRawdate";
				alert(data.msg);
			}
