	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//设置传输数据编码方式
				...
				//创建数据库操作对象
			  	...
			  	System.out.println("执行FindPwdByEmailSvlt");
				 try {  
					 
						String email=request.getParameter("email");  //收件人的email
						String utype=request.getParameter("utype");//获取用户类型
				        ...
				        System.out.println("已配置完邮箱");
				        Session session = Session.getInstance(prop,
								new Authenticator(){
								@Override
								protected PasswordAuthentication getPasswordAuthentication() {
									//返回发件人账号与密码信息
									return new PasswordAuthentication("1370763849@qq.com","xx");
								}
							}
						);
				        System.out.println("准备设置邮件");	
				        //填写信封写信  
